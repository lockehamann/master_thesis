...
见\floatref{fig:testresult.normphi.ra}，
扰动的最大值为5.33368e-3，
最大通量归一化后的相对偏差见\floatref{fig:testresult.normphi.max}，
相对偏差的最大值为7.61166e-3，
总功率归一化后的相对偏差见\floatref{fig:testresult.normphi.power}，
相对偏差的最大值为5.33334e-3，
本文公式计算的相对偏差见\floatref{fig:testresult.normphi.my}，
相对偏差的最大值为5.33397e-3。

\begin{figure}
\centering
\includegraphics[scale=0.6]{normphi_ra.png}
\caption{相对偏差扰动分布图\label{fig:testresult.normphi.ra}}
\end{figure}

\begin{figure}
\centering
\includegraphics[scale=0.6]{normphi_max.png}
\caption{最大通量归一化后的相对偏差分布图\label{fig:testresult.normphi.max}}
\end{figure}

\begin{figure}
\centering
\includegraphics[scale=0.6]{normphi_power.png}
\caption{总功率归一化后的相对偏差分布图\label{fig:testresult.normphi.power}}
\end{figure}

\begin{figure}
\centering
\includegraphics[scale=0.6]{normphi_my.png}
...
\begin{table}
\centering
\caption{静态IAEA PWR三维基准题\ProgramName 计算结果及偏差}
...
时空动力学的计算会产生大量的输出数据，包含模型
所有离散时间和空间网格上的各能群的通量分布和缓发中子源分布，
对于大规模问题所有的数据可达数GB量级，无法完整地列举在文献或报告中，
即使对于空间网格进行归并，只保留组件级的结果，
也往往需要多达数十页才能完整列举。
这样包含完整数据的文献十分少见，
符合本文讨论条件的完整数据结果尚未见到，
无法直接进行精细比较。

为了对程序的正确性进行验证，本文主要采用对比多个程序
计算结果中总功率随时间的变化来进行，这也是文献中
时空动力学程序常用的验证方式。
在此基础上，本文在下一节将讨论不同计算参数下\ProgramName 程序的
细网计算结果的差别，对程序的自洽性进行进行验证，
同时检验其特性是否符合预期，即更细的网格、更短时间步长的条件下，
计算结果的偏差应该更小。
