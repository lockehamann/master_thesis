...
\subsection{整体介绍}
在以上推导的基础上，本文开发了三维扩散时空动力学程序\ProgramName ，
程序主体结构见\floatref{fig:program.structure.whole}，
...
GPU部分使用现在最为通用的CUDA框架进行开发，版本为CUDA 5.0，
并使用了CUDA自带的Thrust通用C++模板库简化和GPU配置相关的代码。
在线性代数库方面，现在业界尚没有较为完整、稳定、高效的稀疏矩阵库，
\ProgramName 程序使用了NVIDIA公司开发的开源稀疏矩阵库CUSP，
因为CUSP支持文本需要的DIA稀疏矩阵存储格式，并且实现较为高效。
CUSP在实现上也是一个C++模板库，大量依赖Thrust的实现，
并且使用了Thrust的显存管理模块，出于性能考虑本文替换了Thrust的显存管理模块。
由于现在Thrust和CUSP并不支持多GPU并行，
所以本程序也仅限使用单GPU进行加速。

在稀疏矩阵格式方面主要使用DIA格式（见\sectionref{sec:gpu.sparseformat.dia}），
并针对扩散方程离散的特点开发了扩散方程矩阵GPU端动态DIA格式生成功能，
以缩短程序总计算时间。
为了对比其他格式，还添加了ELL、CSR、COO格式的实现，
DIA、ELL、CSR、COO格式的线性代数计算功能和各格式间的转化功能使用CUSP库的实现，
本文对其显存管理进行了改进。

在输入文件格式方面，\ProgramName 和一般科学计算程序不同，
使用了Lua语言作为输入文件格式。
...
\ProgramName 和传统确定论成程序一样，采用两段式的输入方式，
即引入材料概念，输入分为两部分：每个材料的各个截面和材料在空间网格上的分布，
前者一般用列表形式给出，后者一般用粗网格上的材料矩阵给出。
这种方式主要针对扩散计算中问题的常见特点：单个组件尺度内往往只有一种材料，
不同组件间的材料也往往有重复的。
但对于全堆均匀化后的截面参数，即各个组件材料不同的情况，这种方式略显冗余。

对于临界计算来说，以上这种两段式的输入方式已经能偶满足实际计算需求，
但对于时空力学问题尚缺乏统一的材料描述方式。
对于时空动力学来说，材料的定义的主体仍然和临界相同，
...
带来的额外的误差。为了提高精度，往往会对这些处于过度状态的网格（甚至包括它们附近的网格），
进行一定的均匀化或插值处理，得到某种过度状态的截面。
而如何进行这方面的均匀化或插值则有多种方法，如按体积加权或按通量加权等，
这些方法并不属于本文的研究范围，所以\ProgramName 最好能提供某种接口来让用户控制
这些网格上的截面计算工作。
...
并使用JIT这样的技术后，其性能才能够勉强接近C语言。
现在这方面的动态语言只有少数的几个，如Lua的LuaJIT实现，
它是目前为止世界上最快的动态语言实现之一\cite{LuaJITHomepage}；
...
预处理截面信息，并传输到GPU显存\;
\If{存在材料分布更新函数MatChange}
{
  \If{上一时间步中，材料分布信息被更新过}
  {
    恢复初始的材料分布信息\;
  }
  在Lua环境内调用材料更新函数MatChange，产生材料分布差分数据\;
  从Lua环境中取回材料分布差分数据\;
  根据材料分布差分数据更新材料分布信息\;
  更新GPU显存上的材料分布信息\;
}
在GPU端跟据材料的截面信息和材料分布信息产生截面在空间网格上的分布\;
...
所以一般计算中会对问题的参数进行修正，使修正后的问题满足$k_\mathrm{eff}=1$，
一般对裂变项进行修正
\begin{align}
  \newcommand{\para}{\pb{\bm{x},t}}
  \nu\Sigma'_{f,g'}\para = \frac{1}{k_\mathrm{eff}}\nu\Sigma_{f,g'}\para
...
使得$k_\mathrm{eff}$的计算值和实际值之间有微小的差别，这跟根源迭代收敛条件有关。
但这个微小的误差会在后续的动力学过程中表现出来，即第一个时间步中总体通量会有一个小的阶跃，
后续时间步中，总体通量也会有小指数上升或小指数下降的趋势，影响时空动力学的计算和结果比较。
...
同临界部分一样，固定源的多群扩散方程可以根据对能群处理的不同而又不同的求解方式，
...
而从显存池中查找适合的显存的时间开销要明显小于向CUDA申请显存的开销，
从而达到了缩短程序运行时间的效果。

对了观察显存分配缓存的影响，这里使用静态IAEA PWR三维算例进行计算，
...
为了排除通量幅值的影响，使用本次迭代的最大通量进行归一化，即
\begin{align}
e_k=\frac{\displaystyle \max_{\bm{k},g}\left|\phi_{\bm{k},g}^{(k)}-\phi_{\bm{k},g}^{(k-1)}\right|}
         {\displaystyle \max_{\bm{k},g}\phi_{\bm{k},g}^{(k)}}
\end{align}